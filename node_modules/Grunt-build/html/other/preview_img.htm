...
<head>
    ...
    <style type="text/css">
        ...
        .preview {margin-top:10px;height:87px}
        .gray {color:#a9a9a9; width: 320px; }
        .cf {zoom:1;}
        .cf:after {clear:both;display:table;content:"";}
        .fl {float:left;}
        .pr {width:64px;text-align:center;margin-left:10px;position:relative;}
        .pr img {width:64px;height:64px;}
        a {text-decoration:none;color:#7f7c77;outline:none;}
        a:hover {text-decoration:underline;}
        a img {border:none;}
        input {outline:none;}
        .file {width:210px; height: 38px; opacity: 0; position: absolute;  left: 0;  top:0; z-index: 10; filter:alpha(opacity=0);}
...
<form id="form" action="" method="post" enctype="multipart/form-data" name="form1" id="form1" class="form">
    ...
    <span class="select-tips">请选择本地文件...</span>
    <input class="upload" type="submit" id="select-img" value="上传">
    <span class="upload-tips">上传</span>
</form>
...
<script type="text/javascript" src="http://ptres.37.com/js/jquery1.8.3.js"></script>
<script type="text/javascript">
        ...
        $.getJSON( $this.attr("href") )
                .done(function( res ) {
                    if ( res.status == "1" ) {
                        ...
                        if ( !len ) {
                            $p.html(
                                    '<p class="gray">先选择格式为jpg/jpeg/png/gif的图片，然后点击“上传”按钮。最多可以传五张图片，总大小不超过 2 M。</p>'
                            );
                        }
    ...
    $( "#form" ).on( "submit", function( e ) {
        ...
        if ( !/(\.gif|\.jp(e)?g|\.png)$/i.test(this.img.value)  ) {
            e.preventDefault();
            return parent.SQ.alert( "请选择选择格式为jpg/jpeg/png/gif的图片^o^" );
        }
        ...
        parent.$( ".service-form-tr-upload").height("200px");
    });
